...
import './ShopChange.css'

class ShopChange extends React.Component {

    static displayName = "ShopChange"
    ...
    inputChanged = (eo) => {
        ...
        if (eo.target.value == '') {
            this.validFunc(eo, true);
        } else {
            this.validFunc(eo, false);
        }
    }
    ...
    validFunc = (eo, error) => {
        switch (eo.target.name) {
            case 'itemName':
                this.setState({ name: eo.target.value, nameError: error });
                break;
            case 'itemPrice':
                this.setState({ price: eo.target.value, priceError: error });
                break;
            case 'itemURL':
                this.setState({ url: eo.target.value, urlError: error });
                break;
            case 'itemQuantity':
                this.setState({ quantity: eo.target.value, quantityError: error });
                break;
        }
    }

    changeItems = (eo) => {
        if (eo.target.value == 'Save') {
